#!/usr/bin/env python3
"""给王虹手写风 HTML 生成“时间轴版”：内联全部样式，注入逐条动画运行时。
...
来源：z-wanghong-handwritten-ppt (tjxj/z-skills, MIT)，由 yh-slides 吸收。
"""
...
def main():
    ...
    parser.add_argument("--source", required=True, type=pathlib.Path, help="源 HTML（王虹手写 PPT 的 index.html）")
    parser.add_argument("--out", type=pathlib.Path, default=None, help="输出 HTML 路径（默认与源同目录的 index-timeline.html）")
    args = parser.parse_args()

    skill_dir = pathlib.Path(__file__).resolve().parent.parent
    source = args.source.resolve()
    out = args.out.resolve() if args.out else (source.parent / "index-timeline.html")
    timeline_css = (skill_dir / "assets/wanghong/timeline.css").read_text(encoding="utf-8")
    timeline_js = (skill_dir / "assets/wanghong/timeline.js").read_text(encoding="utf-8")
