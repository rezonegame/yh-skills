...
class SVGQualityChecker:
    ...
    def _check_text_elements(self, content: str, result: Dict):
        ...
        # Check for overly long single-line text (may need wrapping). Library
        # templates often contain intentionally verbose placeholder labels; keep
        # this warning for concrete projects, where the final copy is known.
        text_matches = re.findall(r'<text[^>]*>([^<]{100,})</text>', content)
        if text_matches and not self.template_mode:
            result['warnings'].append(
    ...
    def _check_image_references(self, content: str, svg_path: Path, result: Dict):
        ...
        for tag_match in img_tag_pattern.finditer(content):
            ...
            if self.template_mode and "{{" in href and "}}" in href:
                # Template skeletons often carry unresolved asset placeholders
                # such as {{LOGO_HEADER}}. They are resolved only after the
                # template is copied into a concrete project, so they should not
                # make the library template fail validation.
                continue
    ...
    def _check_template_contract(self, dir_path: Path,
        ...
        is_chart_library = (dir_path / 'charts_index.json').exists()

        if spec_pages:
            spec_set = set(spec_pages)
            orphan = sorted(on_disk - spec_set - self.TEMPLATE_OPTIONAL_SVG_STEMS)
        ...
        elif is_chart_library:
            # The chart library is indexed by charts_index.json rather than a
            # slide-roster design_spec.md. Per-file SVG validity is still
            # checked above; roster checks apply to layout template folders.
            pass
        else:
            self._template_issues.append((
                'error',
