...
<div class="foot">
  <div class="title">项目名 · CodePilot　|　本地项目地址</div>
...
**严禁使用 emoji**。用本地 Lucide vendor 资源（template.html 已引入）。
...
Magazine 模板默认开启翻页入场动画，由本地 Motion One vendor 资源驱动。`path-c-magazine-seed.html` 会加载输出目录里的 `./assets/vendor/js/motion.min.js`；加载失败时，所有带 `data-anim` 的元素会保持可见，内容不会因为动效失效而不可读。
...
| recipe | 触发方式 | 行为 | 代表布局 |
|---|---|---|---|
| `cascade` | 默认 | 所有 `data-anim` 逐个淡入 | 数据页 / 图文页 / 图片网格 |
| `hero` | `.hero` slide 自动使用 | 慢节奏入场，仪式感更强 | 封面 / 章节幕封 / 问题页 |
| `quote` | `data-animate="quote"` | `data-anim="line"` 逐句揭示 | 大引用 |
| `directional` | `data-animate="directional"` | 左列从左入，右列从右入 | Before / After |
| `pipeline` | `data-animate="pipeline"` | 按右键/空格/滚轮逐步点亮 step | Pipeline |

### 标记原则

- 给 kicker、标题、lead、callout、stat-card、figure、rowline、pillar 这类语义叶子节点加 `data-anim`
- 不要给 `.grid-6` / `.frame` 这类大容器加，除非整块确实要一起出现
- Pipeline 页的 `<section>` 必须加 `data-animate="pipeline"`，每个步骤加 `data-anim="step"`
- 大引用页的 `<section>` 加 `data-animate="quote"`，每行 quote 用 `<span data-anim="line" style="display:block">...`
- 生成后自检 `grep -c 'data-anim' index.html`，如果只有个位数，基本就是漏标
