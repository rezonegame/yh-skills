...
> 来源：吸收自 Open Design 内置的 guizang-ppt 工作流，并结合 huashu-design 的 "20 design philosophies × 5 streams" 思路。这里压缩成 5 个 magazine-flavored 的方向，每个都对应到 `themes.md` 的某一套 + `layouts.md` 的某些组合。使用时只依赖本地化后的方向包。

---

## 何时用这份文档

在 SKILL.md `Step 1-A · 方向选择`：**先让用户在这 5 个方向里挑一个**，再去确认时长 / 受众 / 大纲 / 图片策略。流程是：
