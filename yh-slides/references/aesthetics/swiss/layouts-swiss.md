...
本主题的本地 golden source 是 `assets/seeds/path-c-swiss-seed.html`。上游参考只作为已吸收来源记录,运行时不得依赖个人本地路径或外部仓库。

生成正文页时不要把 Swiss 当成“自由组合的风格包”。默认只能使用 `references/aesthetics/swiss/swiss-layout-lock.md` 登记的 `S01-S22`。每个 slide 都必须在 `<section>` 上写 `data-layout="Sxx"`。
...
1. 同时打开三份页面:当前 `assets/seeds/path-c-swiss-seed.html`、生成页、正在修改的测试 PPT。若有外部参考 PPT,只作为视觉对齐参考,不要依赖任何个人本地路径。
...
| 单图解释论点 / 图文混排 | S22 Image Hero（P23 仅实验时使用） |
| 2-3 张图片/截图/图表证据链 | S15/S16 图片格改造（P24 仅实验时使用） |
...
| 1 张核心图片 + 一段解释 | S22 Image Hero | P23(未登记实验版式,默认禁用) |
| 2-3 张同类图片 | S15/S16 图片格改造 | P24(未登记实验版式,默认禁用) |
