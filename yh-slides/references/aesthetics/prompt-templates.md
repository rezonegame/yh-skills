...
CLI:
```bash
python scripts/generate_image.py extract-style \
  --image reference.png
```
...
**本地化来源说明：**
- 本节吸收了公开 AI slide prompt 资源中的通用方法论。
- 使用 `yh-slides` 时不需要访问这些外部资源；具体模板已经沉淀为本文件中的本地规则。

---

## 10. Public Prompt Resources

| Resource Family | Localized Use | Highlights |
|----------|------|-----------|
| Business prompt libraries | 已吸收为商业、案例、webinar prompt 模式 | Full-type templates: business, case study, webinar |
| Slide smart-guide resources | 已吸收为结构化 prompt 方法 | 8 core templates + prompt methodology |
| Scenario prompt lists | 已吸收为场景化 prompt 分类 | ready-to-use prompts by scenario |
| NotebookLM-style prompt collections | 已吸收为视觉风格 YAML 思路 | visual style YAML templates |
| Viral presentation frameworks | 已吸收为标题、节奏、布局启发 | 3-tier framework with viral layouts |
| Image prompting guides | 已吸收为图像 prompt 六要素 | formula for image prompts |
| AI research assistant guides | 已吸收为多源综合与受众指定方法 | multi-source synthesis and audience targeting |
