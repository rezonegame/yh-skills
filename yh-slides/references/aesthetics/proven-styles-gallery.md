...
>
> 风格样例图见 `assets/style-samples/`，正式索引见 `references/aesthetics/style-samples.md`。样例图用于风格选择和视觉校准，不是最终 deck 的内容素材。
...
| 属性 | 定义 |
|------|------|
| **Philosophy** | 用熟悉物件和空间关系解释抽象概念；像一页认真绘制的中文技术笔记，而不是白板涂鸦或信息卡拼贴 |
| **Colors** | 近白 `#FBFAF5` / 近黑 `#111111` / 淡蓝 `#D9E8F6` / 淡绿 `#DCEAD6` / 淡桃 `#F5DEB8` / 淡紫 `#E4DCF4` |
| **Typography** | 居中克制标题、淡蓝短下划线、小页码；短标签优先，精确文字改走 2C 原生文本框 |
| **Composition** | 大留白、无满页边框；固定外壳，中央语义图约占宽 50–60%、高 35–45% |
| **Visual language** | 黑色细线、轻排线、低饱和标签底、物件隐喻；优先无人，最多一个角落微小人物 |
| **Reference** | `assets/style-samples/ian-handdrawn-technical-anchor.png`，仅作视觉校准；方法来源 Ian Handdrawn PPT |
| **执行路径** | `2B / Path B` 完整栅格页；精确文字、数字和专有名词优先 `2C / Path H` |

**推荐场景：** 技术解释、教学、知识卡片、方法论；作为候选推荐，不设为知识类全局默认。完整契约见 `references/aesthetics/ian-handdrawn-technical.md`。
...
| 教育/培训 | Ian 中文手绘技术解释 | 学習漫画 | Snoopy 温暖漫画 |
| 技术分享 | Ian 中文手绘技术解释 | xkcd 白板 | Ligne Claire |
