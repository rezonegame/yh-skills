...
- HTML → 可编辑 PPTX 导出 → 本地 `yh-slides` 仍优先走 `2A / Path A`、`references/paths/path-workflows.md` 和 `scripts/html2pptx.js`；本文件中提到的上游 `references/editable-pptx.md` / `scripts/export_deck_pptx.mjs` 仅作理念参考，未作为本地默认路径引入。

> **本地适配说明（2026-06-09）**：本文来自 `huashu-design` 的 deck 流程，已吸收 HTML deck、概览墙、PDF 导出和缩略图生成能力。可编辑 PPTX 部分不要覆盖本地 `yh-slides` 既有的 `2A / 2A-S / 2A-T` 路线；如用户明确要求 editable PPTX，回到本地 SKILL.md 的路径选择和 Path A/Path S/Template Fill 规范。
...
极个别情况：HTML 已经写好了才发现要 PPTX。优先回到本地 `2A / Path A`
重写为受限 HTML 中间稿。只有原始 HTML 已不可用、唯一来源只剩页面截图时，
才使用 `2B-R / FigEdit Reconstruction`，细节见
`references/paths/path-workflows.md` 与
...
> 本地适配：`yh-slides` 当前未引入上游 `export_deck_pptx.mjs` 作为默认脚本。需要可编辑 PPTX 时，优先按 `SKILL.md` 的 `2A / Path A`、`2A-S` 或 `2A-T` 选择路线，并读取 `references/paths/path-workflows.md` 与对应 integrations 文档。本节仅保留为上游设计取舍参考。

上游命令示例是 `node scripts/export_deck_pptx.mjs --slides <dir> --out deck.pptx`。本地未将该脚本设为默认入口；需要可编辑 PPTX 时按 `SKILL.md` 的 `2A / Path A`、`2A-S` 或 `2A-T` 路线执行。

工作原理：`html2pptx` 逐元素读 computedStyle 把 DOM 翻译成 PowerPoint 对象（text frame / shape / picture）。文字变成真文本框，PPT 里双击即可编辑。

**硬性约束**（HTML 必须满足，否则该页 skip；本地执行细节见 `references/paths/path-workflows.md`）：
...
如果你的 deck 会长期维护、反复修改、团队协作——建议一开始就选择本地 `2A / Path A`、`2A-S` 或 `2A-T`，而不是先做视觉自由度最高的 HTML 后再补救。具体执行读取 `references/paths/path-workflows.md`、`references/integrations/path-s-svg-native-pptx.md`、`references/integrations/template-fill-pptx.md`。
