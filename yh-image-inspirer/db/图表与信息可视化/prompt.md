...
适用关键词：信息图、图表、可视化、图解、科普、拆解、流程图、时间线、对比图、机制说明、地图
适合任务：科普信息图、机制说明、流程/时间线/对比/拆解图、图文知识卡、桌游规则机制图
不适合任务：纯氛围海报、纯商品摄影、角色立绘、Logo 设计
可作为主库：最终交付物以信息结构、解释和图文模块为核心
可作为辅助库：为商品、桌游、教育、海报提供卖点模块、机制图解和层级结构
常见交叉库：商品与电商、海报与排版、插画与艺术、UI与界面、文档与出版物
必要槽位：图表类型、模块数量、核心信息、受众、比例、语言、是否有参考数据
可默认槽位：默认 3-6 个模块；短标题+短说明；9:16 或 4:5；简体中文
必须确认槽位：是否要求数据准确；是否有必须出现的文字/数字；是科普图还是营销卖点图
优先输出形态：信息图、拆解图、流程图、对比图、时间线、知识卡、地图
默认比例：竖版 9:16/4:5；横向 16:9；方形知识卡 1:1
默认语言：默认简体中文；文字要克制可读
常见误判：信息图不等于大段文字海报；必须控制模块数量和字号层级
关联 recipe / template：recipes/infographic.md；structured-templates.md#publication；structured-templates.md#ui
检索建议：优先检索图表类型、模块数量、主题、信息层级和受众
