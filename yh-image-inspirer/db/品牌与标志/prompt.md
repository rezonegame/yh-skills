...
适用关键词：Logo、品牌、标志、VI、字体、图标、品牌系统、商标、字标、视觉识别、icon
适合任务：Logo 方向、品牌视觉、图标系统、VI 应用、字体气质、品牌资产参考
不适合任务：需要完整电商海报、桌游宣传图、复杂信息图、真实人物摄影的任务
可作为主库：最终交付物是品牌识别、Logo、图标或视觉系统
可作为辅助库：为海报、商品图、系列图提供品牌色、字感、图标和识别约束
常见交叉库：商品与电商、海报与排版、UI与界面、系列生成
必要槽位：品牌名、行业、受众、调性、是否已有 Logo/色彩、禁用颜色、输出形态
可默认槽位：默认简洁、可缩放、少文字；无品牌色时先建议色彩方向
必须确认槽位：是否需要可商用 Logo 成品还是品牌风格参考；是否需保留现有 Logo
优先输出形态：Logo、字标、图标集、VI 展示、品牌 Campaign 视觉
默认比例：Logo 1:1；品牌板 16:9；社媒品牌图 4:5
默认语言：品牌名按用户提供；说明文字默认简体中文
常见误判：不要把品牌设计误做成商品海报；Logo 不应放在复杂背景上不可识别
关联 recipe / template：structured-templates.md#brand；recipes/series-generation.md
检索建议：优先检索行业、符号、字体气质、主色、应用场景
...
![抹茶品牌触点系统视觉板](images/case362.jpg)

```text
Create a premium "Matcha Brand Touchpoint System" visual board for a modern lifestyle brand called "MATCHA MODE". Build a full brand identity system, not a single image.
HERO SCENE: A hyper-realistic matcha drink in a ceramic cup placed on a clean natural surface. Vibrant green matcha foam with micro-bubbles, bamboo whisk nearby, soft natural light.
FULL BRAND SYSTEM: takeout cups, packaging boxes, tote bags, labels, stickers, seals, menu cards with pricing, small typography everywhere, subtle imperfections.
DESIGN LANGUAGE: modern minimalist typography, Japanese-inspired layout, soft green palette, elegant spacing.
```
