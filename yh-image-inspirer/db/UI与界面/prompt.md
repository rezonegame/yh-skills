...
适用关键词：UI、界面、App、网页、手机截图、直播间、仪表盘、控制台、游戏界面、面板、HUD、galgame UI
适合任务：App/网页界面截图、游戏 UI、玩家面板、直播界面、信息面板、工具界面、UI 风格参考
不适合任务：纯商品海报、桌游盒面设计、纯角色立绘、摄影写真、建筑空间渲染
可作为主库：最终交付物是界面、截图、面板、HUD 或系统 UI
可作为辅助库：为海报、信息图、桌游组件提供界面模块、按钮、面板、状态栏或 HUD 语言
常见交叉库：人物与角色、图表与信息可视化、海报与排版、商品与电商
必要槽位：界面类型、使用场景、设备/画幅、核心模块、语言、是否需要真实可读文字
可默认槽位：简体中文、清晰模块层级、少量文字、手机界面默认 9:16、网页/仪表盘默认 16:9
必须确认槽位：是否是真实产品界面还是概念截图；是否需要保留参考图中的 UI 结构或品牌识别
优先输出形态：手机截图、网页首屏、仪表盘、游戏 HUD、玩家面板、直播界面
默认比例：手机 9:16；网页/仪表盘 16:9；面板/卡片 1:1 或 4:3
默认语言：默认简体中文；除非用户要求，不主动加入英文按钮或英文标签
常见误判：用户说卡牌时不要直接当 UI；先判断是牌面正面、玩家面板还是产品效果图
关联 recipe / template：structured-templates.md#ui；recipes/boardgame-ui.md；recipes/boardgame-card.md
检索建议：优先检索界面类型、设备形态、模块名称、信息层级和交互控件
