...
适用关键词：人物、角色、立绘、设定、NPC、头像、动作分解、角色卡、职业、阵营、皮肤、coser
适合任务：角色设定图、半身/全身立绘、人物主视觉、卡牌角色、动作姿态、头像参考
不适合任务：完整 UI 截图、商品详情页、纯 Logo、建筑空间、没有人物主体的信息图
可作为主库：最终交付物以人物/角色识别为核心
可作为辅助库：为卡牌、海报、古风、插画、场景叙事补充角色主体和姿态
常见交叉库：插画与艺术、历史与古风题材、场景与叙事、海报与排版、UI与界面
必要槽位：角色身份、姿态、服装/道具、画风、用途、是否保留参考人物识别
可默认槽位：无画面文字；默认 1:1 或 3:4；角色居中；主体清晰
必须确认槽位：是否需要严格保留参考人物身份；输出是角色图、卡牌主视觉还是宣传海报
优先输出形态：角色设定图、头像、半身像、全身立绘、卡牌主视觉
默认比例：头像 1:1；立绘 3:4；海报人物主视觉 4:5 或 9:16
默认语言：默认不加文字；需要文字时使用简体中文
常见误判：用户说卡牌角色时必须先读 boardgame-card，避免把牌面做成单纯人物插画
关联 recipe / template：recipes/boardgame-card.md；structured-templates.md#illustration；structured-templates.md#scene
检索建议：优先检索角色类型、服装、姿态、道具、风格、用途
...
![高端 3D 收藏玩具头像](images/case378.jpg)
