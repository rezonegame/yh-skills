...
def show_diagnostic_banner(diag: dict):
    ...
    if IS_TTY:
        ...
        # Reddit
        reddit_source = diag.get("reddit_source", "native")
        if reddit_source == "gemini":
            lines.append(f"{Colors.DIM}│{Colors.RESET}  {Colors.GREEN}✅ Reddit{Colors.RESET}    — Gemini API (Search Grounding)       {Colors.DIM}│{Colors.RESET}")
            ...
            lines.append(f"{Colors.DIM}│{Colors.RESET}  {Colors.GREEN}✅ Reddit{Colors.RESET}    — Native (free API, no key needed)    {Colors.DIM}│{Colors.RESET}")
            lines.append(f"{Colors.DIM}│{Colors.RESET}     └─ Add GEMINI_API_KEY for better LLM search     {Colors.DIM}│{Colors.RESET}")
        ...
        reddit_source = diag.get("reddit_source", "native")
        if reddit_source == "gemini":
            lines.append("│  ✅ Reddit    — Gemini API (Search Grounding)       │")
        elif reddit_source == "openai":
            lines.append("│  ✅ Reddit    — OPENAI_API_KEY found                │")
        else:
            lines.append("│  ✅ Reddit    — Native (free API, no key needed)    │")
            lines.append("│     └─ Add GEMINI_API_KEY for better LLM search     │")
