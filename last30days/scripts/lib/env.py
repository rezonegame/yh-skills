...
def get_available_sources(config: Dict[str, Any]) -> str:
    ...
    """
    # Reddit is always available (native fallback), but we check for
    # LLM-backed sources (Gemini/OpenAI) for the 'has_reddit' signal
    has_reddit = bool(config.get('GEMINI_API_KEY') or config.get('OPENAI_API_KEY'))
...
def get_missing_keys(config: Dict[str, Any]) -> str:
    ...
    if has_reddit and has_x and has_web:
        ...
        return 'reddit'  # Missing LLM-backed Reddit (native still works)
    else:
        return 'all'  # Missing everything (native Reddit still available)
...
def validate_sources(requested: str, available: str, include_web: bool = False) -> tuple[str, Optional[str]]:
    ...
    if available == 'web':
        if requested == 'auto':
            # Auto without LLM keys falls back to just web (and native reddit gets added in main)
            return 'web', None
        elif requested == 'web':
            return 'web', None
        elif requested == 'reddit':
            # Reddit is still available via Native fallback
            return 'reddit', None
        else:
    ...
    if requested == 'reddit':
        # Reddit is always available (either via Gemini/OpenAI or Native JSON)
        if include_web:
            return 'reddit-web', None
        return 'reddit', None

    if requested == 'x':
        if available == 'reddit' or available == 'web':
            return 'none', "Requested X but no XAI_API_KEY or Bird CLI available."
        if include_web:
            return 'x-web', None
        return 'x', None
