...
def render_compact(report: schema.Report, limit: int = 15, missing_keys: str = "none") -> str:
    ...
    if report.mode == "web-only":
        ...
        lines.append("- `GEMINI_API_KEY` or `OPENAI_API_KEY` → Reddit threads with engagement signals")
    ...
    if report.mode == "reddit-only" and missing_keys == "x":
        ...
        lines.append("*💡 Tip: Add GEMINI_API_KEY or OPENAI_API_KEY for Reddit data and better triangulation.*")
        lines.append("")
