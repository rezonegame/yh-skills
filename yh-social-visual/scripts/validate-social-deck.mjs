 ...
 *   R8  story contract         ratio, node count, review, sources, and coordinates
 *   R9  story geometry         safe areas, collisions, and non-uniform spacing
 *   R10 story route/text       route span, minimum type, and timeline anti-patterns
 */
...
if (fs.statSync(abs).isDirectory()) {
  htmlPath = fs.existsSync(path.join(abs, "html", "index.html"))
    ? path.join(abs, "html", "index.html")
...
for (const s of sections) {
  const meta = await s.evaluate(el => {
    ...
    let board = el.classList.contains("story-scroll") ? "story-scroll"
              : el.classList.contains("xhs") ? "xhs"
  ...
  if (meta.board === "story-scroll") {
    const story = await s.evaluate(el => {
      const er = el.getBoundingClientRect();
      const nodes = [...el.querySelectorAll("[data-story-node]")];
      const details = nodes.map((node) => {
        const rect = node.getBoundingClientRect();
        const textSizes = [...node.querySelectorAll("h1,h2,h3,h4,p,span")]
        ...
        return {
          ...
          left: rect.left - er.left,
          right: rect.right - er.left,
          top: rect.top - er.top,
          bottom: rect.bottom - er.top,
          centerX: (rect.left + rect.right) / 2 - er.left,
          minFont: textSizes.length ? Math.min(...textSizes) : 0,
        };
      });
      const anchorElements = [...el.querySelectorAll("[data-story-anchor]")];
      const anchors = new Map(anchorElements.map((anchor) => [anchor.dataset.storyAnchor, anchor.getBoundingClientRect()]));
      const textAnchorCollisions = [];
      for (const node of nodes) {
        const anchorId = node.dataset.anchor || "";
        const anchorRect = anchors.get(anchorId);
        if (!anchorRect) continue;
        for (const text of node.querySelectorAll("h1,h2,h3,h4,p,span")) {
          if (!text.textContent.trim()) continue;
          const rect = text.getBoundingClientRect();
          const overlapW = Math.max(0, Math.min(rect.right, anchorRect.right) - Math.max(rect.left, anchorRect.left));
          const overlapH = Math.max(0, Math.min(rect.bottom, anchorRect.bottom) - Math.max(rect.top, anchorRect.top));
          if (overlapW > 4 && overlapH > 4) {
            textAnchorCollisions.push(anchorId || "(missing)");
            break;
          }
        }
      }
      ...
      for (let i = 0; i < details.length; i += 1) {
        for (let j = i + 1; j < details.length; j += 1) {
          ...
          if (overlapW > 8 && overlapH > 8) collisions.push(`${i + 1}/${j + 1}`);
        }
      }
      const centers = details.map((item) => item.centerX).sort((a, b) => a - b);
      ...
      const gapVariance = gaps.length ? gaps.reduce((sum, value) => sum + (value - gapMean) ** 2, 0) / gaps.length : 0;
      const gapCv = gapMean ? Math.sqrt(gapVariance) / gapMean : 0;
      const route = el.querySelector(".story-route");
      const routeRect = route?.getBoundingClientRect();
      const numberedCount = nodes.filter((node) => /^\s*(?:第?\s*\d+\s*[.)、：:]|\d+\s*[.)、])/.test(node.textContent)).length;
      return {
        ratio: el.clientWidth / el.clientHeight,
        reviewed: el.dataset.storyBaseReviewed === "true",
        count: nodes.length,
        missingSources: details.filter((item) => !item.source).length,
        invalidCoords: details.filter((item) => !Number.isFinite(item.x) || !Number.isFinite(item.y) || item.x < 0 || item.x > 1 || item.y < 0 || item.y > 1).length,
        missingAnchors: details.filter((item) => !item.anchor || !anchors.has(item.anchor)).length,
        anchorCount: anchorElements.length,
        unsafe: details.filter((item) => item.left < 48 || item.right > el.clientWidth - 48 || item.top < 36 || item.bottom > el.clientHeight - 36).length,
        collisions,
        textAnchorCollisions,
        gapCv,
        minFont: details.length ? Math.min(...details.map((item) => item.minFont || Infinity)) : 0,
        numberedCount,
        routeWidth: routeRect ? routeRect.width / er.width : 0,
        routeHeight: routeRect ? routeRect.height / er.height : 0,
      };
    });

    if (story.ratio < 2.6 || story.ratio > 3.0) {
      fails.push({ rule: "R8", msg: `story-scroll ratio ${story.ratio.toFixed(2)} outside 2.6:1–3:1`, fix: "use the 2400×900 seed or another canvas inside the allowed ratio" });
    }
    if (story.count < 5 || story.count > 8) {
      fails.push({ rule: "R8", msg: `${story.count} story nodes; expected 5–8`, fix: "re-plan the factual arc before composition" });
    }
    if (!story.reviewed) {
      fails.push({ rule: "R8", msg: "native story base is not marked as visually reviewed", fix: "view the base, verify objects/path/safe regions, then set data-story-base-reviewed=true" });
    }
    if (story.missingSources || story.invalidCoords) {
      fails.push({ rule: "R8", msg: `${story.missingSources} nodes lack data-source; ${story.invalidCoords} nodes have invalid normalized coordinates`, fix: "record a brief source and data-x/data-y in 0–1 for every node" });
    }
    if (story.missingAnchors || story.anchorCount !== story.count) {
      fails.push({ rule: "R8", msg: `${story.missingAnchors} nodes lack a matching object anchor; ${story.anchorCount} anchors for ${story.count} nodes`, fix: "bind every observed object and label with matching data-story-anchor/data-anchor IDs" });
    }
    if (story.unsafe || story.collisions.length || story.textAnchorCollisions.length) {
      fails.push({ rule: "R9", msg: `${story.unsafe} unsafe nodes; ${story.collisions.length} node collisions; ${story.textAnchorCollisions.length} text/object collisions${story.textAnchorCollisions.length ? ` (${story.textAnchorCollisions.join(", ")})` : ""}`, fix: "move labels into observed empty regions without covering objects or each other" });
    }
    if (story.count >= 5 && story.gapCv < 0.08) {
      fails.push({ rule: "R9", msg: `node spacing is mechanically uniform (gap CV ${story.gapCv.toFixed(3)})`, fix: "use an irregular continuous route rather than equally spaced milestones" });
    }
    if (story.routeWidth < 0.75 || story.routeHeight < 0.20) {
      fails.push({ rule: "R10", msg: `story route spans ${Math.round(story.routeWidth * 100)}% width × ${Math.round(story.routeHeight * 100)}% height`, fix: "add one continuous irregular .story-route spanning the visual sequence" });
    }
    if (story.minFont < 26) {
      fails.push({ rule: "R10", msg: `story node minimum type is ${Math.round(story.minFont)}px`, fix: "keep all story node labels and annotations at 26px or larger" });
    }
    if (story.numberedCount >= 2) {
      fails.push({ rule: "R10", msg: `${story.numberedCount} nodes begin with timeline numbering`, fix: "remove numbered dots/steps and label the factual scene directly" });
    }
  }
...
lines.push("");
lines.push(`Legend: R1 overflow | R2 footer collision | R3 swiss bold display | R4 min font | R5 4-band density | R6 h-xl cap | R7 figure margin drift | R8 story contract | R9 story geometry | R10 story route/text`);
